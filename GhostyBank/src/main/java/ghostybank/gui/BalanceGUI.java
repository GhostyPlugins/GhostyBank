...
public class BalanceGUI {
    ...
    public void open(Player viewer, BankData data) {
        ...
        // Total interest + level cap info
        boolean capEnabled = plugin.getConfig().getBoolean("interest.max-total-interest-cap.enabled", false);
        String capDisplay = capEnabled && level.hasInterestCap()
                ? bankManager.formatCurrency(level.getMaxTotalInterest())
                : "∞";
        String levelInterest = bankManager.formatCurrency(data.getLevelInterestEarned());
